...
from dual_transf_split_after_dual import Dual_Transformer
...
class Net(nn.Module):
    def __init__(self, L=512, width=64):
        ...
        self.inp_conv = nn.Conv2d(in_channels=self.in_channels, out_channels=self.width, kernel_size=(1, 3), stride=(1, 2))  # [b, 64, nframes, 512]
        self.inp_norm = nn.LayerNorm(256)
        self.inp_prelu = nn.PReLU(self.width)

        self.dual_transformer = Dual_Transformer(64, 64, num_layers=5)  # # [b, 64, nframes, 8]
        self.output_after_dual1 = nn.Sequential(nn.PReLU(),
                                    nn.Conv2d(32, 64, 1)
                                    )
        self.output_after_dual2 = nn.Sequential(nn.PReLU(),
                                    nn.Conv2d(32, 64, 1)
                                    )
        # gated output layer
        ...
        # ****---echo add ----
        self.highfreqconv = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=1)
        self.highfreqtanh = nn.Tanh()
        # ****---echo add ----
    ...
    def forward(self, x):
        ...
        x1 = self.inp_prelu(self.inp_norm(self.inp_conv(self.pad1(x))))  # [b, 64, num_frames, frame_size]

        out = self.dual_transformer(x1)  # [b, 64, num_frames, 256]
        out1 = self.output_after_dual1(out)
        out2 = self.output_after_dual2(out)

        out1 = self.output1(out1) * self.output2(out1)  # mask [b, 64, num_frames, 256]
        out2 = self.output11(out2) * self.output22(out2)
        # ****---echo change ----

        out1 = self.maskrelu(self.maskconv(out1))
        out2 = self.highfreqtanh(self.highfreqconv(out2))
        out = x1 * out1 + out2
