...
# file path
train_file_list_path = './female2_train_file_list'
validation_file_list_path = './female2_validation_file_list'
...
for epoch in range(start_epoch, max_epochs):
    ...
    metrics = {'train': ave_train_loss, 'valid': avg_eval, 'pest': avg_pesq, 'stoi': avg_stoi, 'snr': avg_snr}
    ...
    epoch + 1, max_epochs, index + 1, len(train_loader), ave_train_loss, avg_eval))
    ...
    if avg_eval < best_val_loss:
        ...
        is_best_model = False
    metrics.update({'best': best_val_loss})
    json.dump(history, open(history_file, "w"), indent=2)
